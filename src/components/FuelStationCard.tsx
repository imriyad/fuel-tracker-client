...
export default function FuelStationCard({ name, status, distance, price, address, isAdmin, onEdit, onDelete }: StationProps) {
  const statusConfig = {
  ...
  const { color, label } = statusConfig[status as keyof typeof statusConfig] || statusConfig['In Stock'];
